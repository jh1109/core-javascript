...
// -> second 함수에 작성된 내용과 함께 second 외부에 있는 환경에 second를 통해 first 내용도 접근가능하다, 기억한다는 의미
...
function first(){
  ...
  return second;
  // 이렇게 하면 second 함수 그 자체가 나옴. first()를 하면
  //그럼 second 함수까지 실행하려면?
  //first()() 해주면 됨
  // 근데 보통 이렇게 안쓰고 아래처럼 변수를 만들어 함수 그 자체를 넣어주는데
  // 이걸.. closure라고 함!
...
//count를 함수 내에 작성해놨기 때문에 외부에서 count를 가져다가 쓸 수 없고,
// 바꿔줘도 영향을 안줌.
//그래서 이런식으로 작성해서 함수를 보호하고 전역의 오염을 막아주는 거임
//또 이렇게 작성하면 counter가 number()를 계속 참고하고 있는 상태이므로
//가비지컬렉션이 접근을 못함. 가비지컬렉션은 아 얘가 계속 살아있는 애구나 라고 인식함.
// 그래서 묶어놨다의 의미로 closure라고 이름을 한거임.

//"많이 하고 많이 보면 작성할 수 있을거임.."
